...
function openScores() {
    ...
    for (let i = 1; i <= 4; i++) {
        if (localStorage.getItem(`1${i}level`, points) == null)
            scoreLevels[i].innerHTML = "-"
        else
            scoreLevels[i].innerHTML = localStorage.getItem(`1${i}level`, points);
    }

    for (let i = 5; i <= 8; i++) {
        if (localStorage.getItem(`2${i-5}level`, points) == null)
            scoreLevels[i].innerHTML = "-"
        else
            scoreLevels[i].innerHTML = localStorage.getItem(`2${i-5}level`, points);
    }

    for (let i = 9; i <= 12; i++) {
        if (localStorage.getItem(`2${i-9}level`, points) == null)
            scoreLevels[i].innerHTML = "-"
        else
            scoreLevels[i].innerHTML = localStorage.getItem(`2${i-9}level`, points);
    }
...
function levelFailed() {
    ...
    switch (levelnum) {
        case 1:
            if (penalty[0] != 1 && levelsuc[0] != 1 && points - 1 >= 0 ){
                penalty[0] = 1;
                points -= getSumPenalty();
            }

            gameResultPoints.innerHTML = `${points}/10`;
            gameResultPenalty.innerHTML = `${getSumPenalty()}`;
        break;
        case 2:
            if (penalty[1] != 1 && levelsuc[1] != 1 && points - 1 >= 0){
                penalty[1]++;
                points -= getSumPenalty();
            }
        ...
        break;
        case 3:
            if (penalty[2] != 1 && levelsuc[2] != 1 && points - 1 >= 0){
                penalty[2]++;
                points -= getSumPenalty();
            }
        ...
        break;
        case 4:
            if (penalty[3] != 1 && levelsuc[3] != 1 && points - 1 >= 0){
                penalty[3]++;
                points -= getSumPenalty();
            }
